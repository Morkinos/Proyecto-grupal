...
function obtenerCancion($idSong) {
    global $db;
    $query = "SELECT idSong, idAlbum, title, duration, demo_path, full_path, price FROM songs WHERE idSong = ?";
    $stmt = $db->prepare($query);
...
function actualizarCancion() {
    ...
    $query = "UPDATE Avenger_song SET idAlbum = :idAlbum, title = :title, duration = :duration, demo_path = :demo_path, full_path = :full_path, price = :precio WHERE idSong = :idSong";
    $stmt = $db->prepare($query);
    $stmt->bindParam(":idAlbum", $data->idAlbum);
    $stmt->bindParam(":title", $data->title);
...
function borrarCancion() {
    ...
    $query = "DELETE FROM songs WHERE idSong = :idSong";
    $stmt = $db->prepare($query);
    $stmt->bindParam(":idSong", $data->idSong);
